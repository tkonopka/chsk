...
import { ChartQuantileHorizontalDecorator, onQuantilesClick } from './decorators'
...
`Quantiles` is intended to be used within a `Quantile` component. The `Quantiles` component is the one that draws the boxes and whiskers, but much of the layout is determined by `Quantile`.
