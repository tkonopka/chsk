...
When the scale on the value axis is set to automatic, it will automatically rescale/adjust when the data changes or when certain keys are toggled using a legend (below, left). The automatic rescaling can be disabled using prop `autoRescale` (below, right).
