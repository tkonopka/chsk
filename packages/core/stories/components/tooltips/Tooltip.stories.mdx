...
`Tooltip` determines its target position from the tooltip context via the `useTooltip` hook.
The target position is often the location of a mouse pointer.
Further, the tooltip can be offset from the target position with prop `position`.
...
The examples below display tooltips that have: compact padding, ample padding,
and a title line.
