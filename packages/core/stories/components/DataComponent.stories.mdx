...
In add-on packages, e.g. `@chask/xy` and `@chask/band`, some chart components are assigned
...
The full code is available in the [source repository](https://github.com/tkonopka/chask/blob/main/packages/core/stories/components/CustomDataComponent.tsx).
