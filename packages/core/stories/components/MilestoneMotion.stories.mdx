...
<Meta title="Core/Components/MilestoneMotion" />
export const Template = args => <MilestoneMotion {...args} />

`MilestoneMotion` is an organizational component. It does not draw anything itself, but it controls when its children elements appear and disappear given chart milestones.

<ArgsTable of={MilestoneMotion} />

Charts keep track of a set of 'milestones', which can be toggled, for example using buttons. `MilestoneMotion` components detect changes in the milestones and trigger the visibility of their children elements.

Examples on this page involve multiple components, including external buttons. For this reason, the code is more than a few lines long (the complete source code is available in the github repository). Despite length, however, the main usage of `MilestoneMotion` is to wrap other chart components. In the examples below, `MilestoneMotion` components are used to animate visible of chart axes.
