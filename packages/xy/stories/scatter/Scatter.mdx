...
Alternatively, the props can be arbitrary functions that compute numeric x- and y- values.

Prop `k` specifies another accessor that is used to associated each data point with a key/index.
In many use-cases, it is not necessary to specify this prop, and the relevant keys/indexes will be provided automatically.
However, in use cases where `Scatter` is updated with different subsets of a large dataset,
specifying `k` explicitly helps to prevent unnecessary point animations across updates.
