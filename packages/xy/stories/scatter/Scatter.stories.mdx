...
By default, `Scatter` assumes all data points should have equal size. The size of the symbols can be adjusted via prop `scaleSize`.

To apply different symbol sizes to each point, set `valueSize` to a string key that carries a size value in the original data. This information is used together with the specification for a size scale, `scaleSize`.
