...
<Meta title="Addons/XY/ScatterSeries" />
...
The prop `layers` controls the amount of information that is displayed about a series. Setting the prop to an array `['curve', 'points']`, for example, can display a curve and data points. An array with an additional `'area'` would also create a polygon.
