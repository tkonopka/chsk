...
From the base position computed from the `x` and from the series data, the label can be shifted using prop `offset`.
...
The label can be rotated with prop `angle`. Alternatively, a prop `autoRotate` can compute a relevant rotation angle.
