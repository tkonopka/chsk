...
export const processData3 = (result: Array<VennProcessedDataItem>, separation: number) => {
    ...
    const p01 = addPoints(result[0].center, outerPoint)
    const p10 = addPoints(result[0].center, innerPoint)
    const p12 = addPoints(result[1].center, rotatePoint(outerPoint, alpha))
    const p21 = addPoints(result[1].center, rotatePoint(innerPoint, alpha))
    const p20 = addPoints(result[2].center, rotatePoint(outerPoint, -alpha))
    const p02 = addPoints(result[2].center, rotatePoint(innerPoint, -alpha))
